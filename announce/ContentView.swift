// 1.現在の時刻表示：基準時刻同期対応済み
// 2.次の放送までのカウントダウン表示：1sギャップ修正済み
// 3.次の放送内容表示
// 4.自動再生開始／停止ボタン
// 5.ミュートON/OFFスイッチ
// 6.手動音声ボタン：「閲覧」「開始」「終了」
// 7.スケジュール：領域1・5の1回分、領域1・5のみ、領域2・3・4
...
//    @State private var demoMode: DemoMode = .demo1
    @State private var demoMode: DemoMode = .demo2   // ★ ここを .demo2 に変更
//    @State private var demoMode: DemoMode = .demo3
    @State private var volume: Double = 1.0   // 0.0〜1.0
    @State private var timeOffset: TimeInterval = 0   // ★追加: 基準時刻との差（秒）
...
//                    .cornerRadius(15)
                    ...
                    HStack {
                        ...
                        Toggle("", isOn: $isMuted)
                            ...
                            .onChange(of: isMuted) { newValue in   // iOS 14〜16 互換
...
//                                audioManager.setVolume(Float(volume), isMuted: isMuted)
//                            }
...
//                    .padding(.horizontal)
                    ...
                    VStack(spacing: 8) {
                        HStack(spacing: 12) {
                            Button("閲覧") {
                                audioManager.play(resourceName: "02 課題読む", isMuted: isMuted)
                            }
                            .buttonStyle(.borderedProminent)

                            Button("開始") {
                                audioManager.play(resourceName: "03 課題開始", isMuted: isMuted)
                            }
                            .buttonStyle(.borderedProminent)

                            Button("終了") {
                                audioManager.play(resourceName: "06 課題終了", isMuted: isMuted)
                            }
                            .buttonStyle(.borderedProminent)
                        }
                    }
...
//            }
            ...
            .onReceive(timer) { _ in
                let rawNow = Date()
                let syncedNow = rawNow.addingTimeInterval(timeOffset)  // ★基準時刻に補正
                currentTime = Date()
                currentTime = syncedNow
                if isRunning { updateSchedule() }
//                if isRunning { updateSchedule(currentNow: syncedNow) }
            }
            .onAppear {
                setupAudio()
                
                // ★ 基準時刻（日本）とのオフセットを取得
                // ここでは NICT の NTP サーバ例。任意の NTP サーバに変えてよいです。
                Clock.sync(from: "ntp.nict.jp") { date, offset in
                    // offset: デバイス時刻との差（秒）
                    timeOffset = offset
                    print("NTP synced. offset = \(offset) sec")
                }
    ...
    private func updateSchedule() {
        let futureItems = schedule.filter { $0.date > currentTime }
//        let futureItems = schedule.filter { $0.date > currentNow }
...
//        let diff = nextItem.date.timeIntervalSince(currentNow)
        // diff: 実際の残り秒数（小数）

        // ① 表示用は常に +1 秒（マイナスにはしない）
        let displaySeconds = max(Int(diff) + 1, 0)
        countdownSeconds = displaySeconds
        nextDisplayName = nextItem.displayName

        // ② 次のティックまでに 0 を切りそうなら、このタイミングで再生
        //    （タイマー間隔は 1秒前提）
        if diff <= 1 {
            playAudio(nextItem)
        }
